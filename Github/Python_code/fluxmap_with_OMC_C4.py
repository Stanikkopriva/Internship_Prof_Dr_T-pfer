...
fluxdata_df = pd.read_csv("C:/Users/stani/OneDrive/Dokumente/Uni stuff/Etagen praktikum/Python/Csv/C4_Flux_Solution.csv")
...
for i in range(len(list(match_dict.keys()))):
    if "[M]" in list(match_dict.values())[i] or "[MB]" in list(match_dict.values())[i]:
        ...
        path_fluxes.append(fluxdata_df.loc[list(match_dict.values())[i] ,"fluxes"]*1.571 )
...
#(chatgpt)increasing values of list, perfrming a min max normalisation
"""
# Example list
values = path_fluxes
...
print(flux_df)
flux_df.to_excel(r'C:/Users/stani/OneDrive/Dokumente/Uni stuff/Etagen praktikum/Python/Flux_path_excel/flux_path_increased.xlsx', index=False)
#with pd.option_context('display.max_rows', None, 'display.max_columns', None):  # more options can be specified also
    #print(flux_df)
 

#in dataframe fluxdata_match_df assign minmax-normalized fluxvalues to flux_values and direction values to direction


fluxdata_match_df = fluxdata_match_df.assign(flux_values = norm_path_fluxes_minmax, direction = direction)
print(fluxdata_match_df)
...
def b_g(cmap=colormap, low=0, high=0, dataframe = flux_df):
    ...
    fig.savefig("C:/Users/stani/OneDrive/Dokumente/Uni stuff/Etagen praktikum/Python/Colorbar/OMC_Colorbar_3.svg", format='svg', bbox_inches = 'tight')
